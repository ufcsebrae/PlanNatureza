...
try:
    # Ajuste para importar a função de carregamento de gerentes do local correto
...
def gerar_relatorio_para_unidade(unidade_antiga: str, unidade_nova: str, df_base_total: pd.DataFrame):
    ...
    # --- Geração dos Gráficos Plotly e Dados para Acordeão ---
    ...
    heatmap_html = '<div class="hidden md:flex items-center justify-center h-full text-center text-gray-500">Sem dados de projetos exclusivos para exibir o heatmap.</div>'
    accordion_data = []
    ...
    if not df_exclusivos.empty:
        ...
        # HEATMAP (Desktop) e DADOS PARA ACORDEÃO (Mobile)
        df_heat = df_sun.copy() # Reutiliza o df_sun que já tem os dados necessários
        if not df_heat.empty:
            ...
            if not pivot_df.empty:
                ...
                for projeto in pivot_df.index:
                    project_data = {'name': projeto, 'natures': []}
                    for natureza in pivot_df.columns:
                        value = pivot_df.loc[projeto, natureza]
                        if pd.notna(value):
                            project_data['natures'].append({'name': natureza, 'value': value})
                    accordion_data.append(project_data)

    kpi_dict["__SUNBURST_PLACEHOLDER__"] = sunburst_html
    kpi_dict["__HEATMAP_PLACEHOLDER__"] = heatmap_html
    kpi_dict["__HEATMAP_ACCORDION_DATA__"] = json.dumps(accordion_data)
    ...
    try:
        ...
        json_string_chartjs = json.dumps(dados_graficos, indent=None, ensure_ascii=False)
        final_html = final_html.replace('__JSON_DATA_PLACEHOLDER__', json_string_chartjs)
